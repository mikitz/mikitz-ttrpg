async function generateBattleMap(battleMapData, seed) {
    ...
    if (battleMapData) {
        // Battle Map Data is passed through if we're redrawing a map
        const gridType = battleMapData.grid_type;
        if (gridType == "square")
            generateSquareGridBattleMap(null, null, battleMapData);
    ...
    // Labels
    const terrainLabelsOpacity = getLabelAndTileOpacity(
        "terrain-checkbox",
        "terrain-opacity-input"
    );
    const coordinatesLabelsOpacity = getLabelAndTileOpacity(
        "coordinates-checkbox",
        "coordinates-opacity-input"
    );
    const coverLabelsOpacity = getLabelAndTileOpacity(
        "cover-checkbox",
        "cover-opacity-input"
    );
    const elevationLabelsOpacity = getLabelAndTileOpacity(
        "elevation-checkbox",
        "elevation-opacity-input"
    );
    const labelsArray = [
    ...
    const tableCliff = cliff
        ? await db.bmg_cliff_probabilities.toArray()
        : null;
    tableCoverType = convertPropertiesArrayToSeedRollableObject(
        tableCoverType,
        "COVER_TYPE",
        "PROBABILITY"
    );
    tableTerrain = convertObjectToSeedRollableObject(
        tableTerrain.find((e) => e.BIOME == biomeUpper)
    );
    tableDiffTerrain = convertObjectToSeedRollableObject(
        tableDiffTerrain.find((e) => e.BIOME == biomeUpper)
    );
    tableCover = convertObjectToSeedRollableObject(
        tableCover.find((e) => e.BIOME == biomeUpper)
    );
    tableTerrainClassification = convertObjectToSeedRollableObject(
        tableTerrainClassification.find((e) => e.BIOME == biomeUpper)
    );
    const specialFeaturesTables = [
        tableCover,
        tableDiffTerrain,
        tableTerrain,
        tableCoverType,
    ...
    // Check Dimensions
    const isDimensionsCompatible = checkForCompatibleDimensionsBoolean(
        width,
        height,
        gridSize
    );
    if (isDimensionsCompatible)
        return alert(
            `Width / Grid Size = ${widthDividedByGridSize} and Height / Grid Size = ${heightDividedByGridSize}. At least one of these quotients is not a whole number. Please ensure that each of these quotients is a whole number, I.E. no decimal places.`
        );
    // Special Features
    const cliffDieSize = cliff
        ? tableCliff.find((elem) => elem.BIOME == biomeLower).DIE_SIZE
        : null;
    const hillDieSize = hill
        ? tableHill.find((elem) => elem.BIOME == biomeLower).DIE_SIZE
        : null;
    const riverWidth = river ? getRiverWidth(height, seed) : null;
    const beginRiver = river
        ? determinePathBeginningCoordinates(height, width, seed)
        : null;
    const roadWidth = road ? getRoadWidth(seed) : null;
    const beginRoad = road
        ? determinePathBeginningCoordinates(height, width, seed)
        : null;
    const specialFeaturesMiscData = [
        cliffDieSize,
        hillDieSize,
        riverWidth,
        beginRiver,
    ...
    document.getElementById("coordinates").style.opacity =
        coordinatesLabelsOpacity;
    ...
    const data = [
        userInputsArray,
        specialFeaturesBooleanArray,
        specialFeaturesMiscData,
        colorData,
        specialFeaturesTables,
        labelsArray,
    ];
    if (gridType == "square")
        battleMapData = await generateSquareGridBattleMap(
            data,
            seed,
            null,
            colorData,
        ...
        battleMapData = await generateHexBattleMap(
            data,
            seed,
            null,
            colorData,
    ...
    console.log(
        "🚀 ~ file: index.js:332 ~ generateBattleMap ~ formattedTime:",
        formattedTime
    );
...
/* =======================
        ...
        Functions
======================= */
async function generateSquareGridBattleMap(
    data,
    seed,
    battleMapData,
    colorData,
    ...
    for (var i = 0; i < width; i++) {
        for (var j = 0; j < height; j++) {
            ...
            fillCenterOutAnimation(
                ...
                tile.color,
                false
            ); // Animation = false b/c it's too slow with big maps
...
function generateHexBattleMap(data, seed, battleMapData, colorData, contexts) {
    const hexOrientation = document.getElementById("hex-type").value;
    const PPI = parseInt(document.getElementById("grid-size"));
    ...
    if (hexOrientation.includes("row") && direction == 1)
        transformedDirection = 2;
    if (hexOrientation.includes("row") && direction == 2)
        transformedDirection = 3;
    if (hexOrientation.includes("row") && direction == 3)
        transformedDirection = 4;
    if (hexOrientation.includes("row") && direction == 4)
...
function determineDirection(directionOrientation, seed, direction) {
    ...
    if (directionOrientation == "vertical") {
        if (rng <= 0.5) return DIRECTIONS.VERTICAL_BOTTOM_RIGHT_TO_TOP_LEFT;
        else if (rng >= 0.51 && rng <= 1.0)
            return DIRECTIONS.VERTICAL_TOP_LEFT_TO_BOTTOM_RIGHT;
    } else if (directionOrientation == "horizontal") {
        if (rng <= 0.5) return DIRECTIONS.HORIZONTAL_TOP_LEFT_TO_BOTTOM_RIGHT;
        else if (rng >= 0.51 && rng <= 1.0)
            return DIRECTIONS.HORIZONTAL_BOTTOM_RIGHT_TO_TOP_LEFT;
    } else if (directionOrientation == "random") {
        if (rng <= 0.25) return DIRECTIONS.VERTICAL_BOTTOM_RIGHT_TO_TOP_LEFT;
        else if (rng >= 0.26 && rng <= 0.5)
            return DIRECTIONS.VERTICAL_TOP_LEFT_TO_BOTTOM_RIGHT;
        else if (rng >= 0.51 && rng <= 0.75)
            return DIRECTIONS.HORIZONTAL_TOP_LEFT_TO_BOTTOM_RIGHT;
        else if (rng >= 0.76 && rng <= 1.0)
            return DIRECTIONS.HORIZONTAL_BOTTOM_RIGHT_TO_TOP_LEFT;
    }
}
...
function drawKey(elementID, colorData) {
...
function updateProgressElement(element, currentTile, totalTiles) {
    element.innerText = `Processing Tile... ${currentTile} / ${totalTiles.toLocaleString()} (${parseInt(
        (currentTile / totalTiles) * 100
    )}%)`;
}
function getRiverWidth(height, seed) {
    // Function to determine the river's width
    let rng = seed();
    return Math.floor((rng * height) / 3) + 1;
}
function getRoadWidth(seed) {
