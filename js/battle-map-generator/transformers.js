async function transformBattleMap(vector) {
    ...
    const battleMapData = await db.bmg_maps.get(
        localStorage.getItem("current-battle-map-id")
    );
    const map = battleMapData.td;
    const ppi = battleMapData.ppi;
    let seed = getSeed("seed", [
        battleMapData.biome,
        battleMapData.landform,
        battleMapData.plane,
        battleMapData.climate,
        battleMapData.season,
        battleMapData.w,
        battleMapData.h,
    ]);
    seed = seed[0];
